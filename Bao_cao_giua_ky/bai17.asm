...
extract_loop:
 ...
 blt $t1, 48, extract_loop #Neu gia tri luu trong $t4 < 48 (Ma ASCII cua 0) => khong phai ky tu so => chuyen sang duyet phan tu tiep theo trong chuoi
 bgt $t1, 57, extract_loop #Neu gia tri luu trong $t4 > 57 (Ma ASCII cua 9) => khong phai ky tu so => chuyen sang duyet phan tu tiep theo trong chuoi
...
printf_loop:
 addi $t2, $t2, -1 # Giam con tro toi ky tu trong stack di 1 => Tiep tuc xet den ky tu lien trai. O day ta tru di 1 luon do qua buoc trich xuat tren
 #thi con tro $t2 tro toi vi tri nam ngay ben phai ky tu cuoi cung duoc luu vao stack
 li $v0, 11
 lb $a0, 0($t2) #Luu gia tri duoc luu trong dia chi $t2 vao $a0 de in ra man hinh
 syscall
 beq $t2, $sp, end_printf_loop #Neu $t2, $sp tro toi cung 1 vi tri (ky tu nay da in ra man hinh) thi ket thuc qua trinh in ky tu trong stack ra man hinh
